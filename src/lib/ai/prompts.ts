...
import { WRITING_STYLES, DEFAULT_WRITING_STYLE, LESSONS, DEFAULT_LESSON } from "@/lib/styles";
...
// Priority tiers for how frequently family members should appear
...
function formatFamilyContext(members: FamilyMember[]): string {
  ...
  if (tier1.length > 0) {
    ...
    context += tier1.map((m) => `- ${m.name} (${m.role})`).join("\n");
    context += "\n\n";
  }

  if (tier2.length > 0) {
    context += "EXTENDED FAMILY (can appear occasionally — a few pages):\n";
    context += tier2.map((m) => `- ${m.name} (${m.role})`).join("\n");
    context += "\n\n";
  }

  if (tier3.length > 0) {
    context += "OTHER CHARACTERS (cameo appearances — 1-2 pages at most):\n";
    context += tier3.map((m) => `- ${m.name} (${m.role})`).join("\n");
    context += "\n";
  }
...
export function buildStorySystemPrompt(context: {
  ...
  // Resolve writing style instructions
...
IMPORTANT — FAMILY MEMBER USAGE:
- You do NOT need to include every family member in every story.
- Parents and siblings (brother/sister) are core characters — include them naturally and frequently.
- Grandparents and pets can appear occasionally, perhaps in a few scenes.
- Aunts, uncles, and others should only make brief cameo appearances if the story calls for it.
- Choose which family members fit the story's theme naturally. A space adventure might feature a sibling co-pilot, while a garden story might feature grandma.
...
- Page 16: Closing page — a warm goodnight message, "The End", and a cozy final scene
...
CHARACTER SHEET:
Before writing the story, create a CHARACTER SHEET that defines the exact visual appearance of every character and important recurring item. This sheet ensures visual consistency across ALL 16 illustrations.
...
2. "pages": an array of exactly 16 page objects
