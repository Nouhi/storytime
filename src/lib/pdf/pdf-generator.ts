...
export async function generatePdf(
  ...
  for (const storyPage of pages) {
    ...
    if (isCover) {
      ...
      const bedtimeMatch = storyPage.text.match(/^(.+?)\s*[-:,]?\s*(a bedtime story.*)$/i);
      const separatorMatch = !bedtimeMatch && storyPage.text.match(/^(.+?)\s*[-:]\s*(.+)$/);
      ...
      if (match) {
        ...
        drawCenteredText(page, storyPage.text, fontBold, fontBold, 22, textStartY, TEXT_COLOR);
      }
    } else if (isEnd) {
      drawCenteredText(page, storyPage.text, fontBoldItalic, fontBoldItalic, 15, textStartY, TEXT_COLOR);
    } else {
      drawCenteredText(page, storyPage.text, font, font, 13, textStartY, TEXT_COLOR);
    }
  }
