...
import { generateBookletPdf } from "../lib/pdf/booklet-pdf-generator.js";
...
// =============================================================================
// Booklet PDF routers (saddle-stitch imposition)
// =============================================================================

// Router for fresh story booklet PDF: GET /api/generate/:storyId/booklet-pdf
export const generateBookletPdfRouter = Router();

generateBookletPdfRouter.get("/:storyId/booklet-pdf", async (req, res) => {
  const { storyId } = req.params;
  const session = getSession(storyId);
  ...
  const title = storyPages[0]?.text || "Story";
  const pdfBuffer = await generateBookletPdf(title, storyPages, images);

  const filename = `storytime-booklet-${Date.now()}.pdf`;
  res.set("Content-Type", "application/pdf");
  res.set("Content-Disposition", `attachment; filename="${filename}"`);
  res.set("Content-Length", pdfBuffer.length.toString());
  res.send(Buffer.from(pdfBuffer));
...
// Router for historical story booklet PDF: GET /api/story-history/:id/booklet-pdf
export const historyBookletPdfRouter = Router();

historyBookletPdfRouter.get("/:id/booklet-pdf", async (req, res) => {
  const id = parseInt(req.params.id, 10);
  ...
  const title = story.title;
  const pdfBuffer = await generateBookletPdf(title, pages, images);

  const safeName = title
    .replace(/[^a-zA-Z0-9\s-]/g, "")
    .replace(/\s+/g, "-")
    .slice(0, 60);
  const filename = `${safeName || "story"}-booklet.pdf`;
